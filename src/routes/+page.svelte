...
<Background/>
	...
	<div class="absolute inset-0 z-30 flex items-center justify-center h-screen">
		<div class=" p-8 sm:max-w-full max-w-[600px] mx-auto">
			...
			<Text text="Filaïcht SINGAÏNY" split={false} baseDelay={0.5}
						className="leading-[4.70rem] text-7xl text-teal-950" />
	
			<!-- subtitle -->
			<div class="hidden md:block">
				<Text text={text} className="text-xl text-teal-900" baseDelay={2} />
			</div>
			<div class="md:hidden block">
				<Text text={SmallScreenText1} className="text-xl" baseDelay={2} />
				<Text text={SmallScreenText2} className="text-xl" baseDelay={3} />
			</div>

			<!-- Sub-section "Mes projets" -->
			<div class="mt-12">
				<div class="flex items-center">

					<Text split={false} baseDelay={3} text="Mes projets"
								className="w-full text-4xl whitespace-nowrap text-teal-950" />
